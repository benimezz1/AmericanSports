(function () {
    ...
    return `
      <header>
        <div class="container navbar">
          ...
          <nav>${links}</nav>
          <button id="themeBtn" class="theme-toggle" type="button">◐</button>
        </div>
      </header>
  ...
  function renderSidebar({ mode, league, currentPage }) {
    ...
    const modeTitle = league ? league : 'Esta Liga';
    const contextual = getContextualItems(league);
    const items = mode === 'context' && contextual.length ? contextual : GLOBAL_SIDEBAR_ITEMS;

    sidebar.innerHTML = `
      <div class="sidebar-toggle">
        <button class="pill ${mode === 'global' ? 'active' : ''}" data-sidebar-mode="global">Global</button>
        <button class="pill ${mode === 'context' ? 'active' : ''}" data-sidebar-mode="context">${modeTitle}</button>
      </div>
  ...
  function renderHome(root, data, state) {
    ...
    const leagueCards = leaguesOrdered
      .map((league) => {
        const teams = data.teams.filter((team) => team.league === league).slice(0, 2);
        return card(
          league,
          teams.map((team) => team.name).join(' • '),
          `<div class="meta"><a href="${league.toLowerCase()}.html">Abrir hub</a></div>`
        );
      })
      .join('');
    ...
    const upcoming = Sorter.sortByUserPriority(data.games.filter((game) => game.status === 'upcoming'), state)
      ...
      .map((game) => {
        ...
        return card(`${away} @ ${home}`, new Date(game.datetime).toLocaleString('pt-BR'), `<div class="meta">${teamLink(game.league, game.teamAway, away)} • ${teamLink(game.league, game.teamHome, home)}</div>`);
      })
      .join('');

    root.innerHTML = `
      <section class="section"><div class="section-head"><div><h2>Home</h2><p>Hub com prioridade inteligente de conteúdo.</p></div></div>
        <div class="grid">${leagueCards}</div>
      </section>
      <section class="section"><div class="section-head"><div><h2>Próximos Jogos</h2></div><a href="games.html">Ver todos</a></div>
  ...
  function renderLeaguePage(root, league, data, state) {
    ...
    const news = Sorter.sortByUserPriority(data.news.filter((item) => item.league === league), state)
      .map((item) => card(item.title, item.summary, `<div class="meta">${new Date(item.date).toLocaleDateString('pt-BR')}</div>`))
      .join('');

    const teamCards = Sorter.sortByUserPriority(teams.map((team) => ({ ...team, teams: [team.id] })), state)
      .map((team) => card(team.name, `Sigla: ${team.abbreviation}`, `<div class="meta">${teamLink(league, team.id, 'Abrir time')}</div>`))
      .join('');

    root.innerHTML = `
      <section class="section"><div class="section-head"><div><h2>${league}</h2><p>Página modular da liga.</p></div></div>
        <div class="grid">${teamCards}</div>
      </section>
      <section class="section"><div class="section-head"><div><h2>Notícias ${league}</h2></div></div>
  ...
  function renderGames(root, data, state, leagueFilter) {
    ...
    const cards = Sorter.sortByUserPriority(filtered, state).map((game) => {
      ...
      const away = getTeamName(data.teams, game.teamAway);
      return card(
        `${game.league} • ${away} @ ${home}`,
        `${new Date(game.datetime).toLocaleString('pt-BR')} • ${game.status}`,
        `<div class="meta">${teamLink(game.league, game.teamAway, away)} • ${teamLink(game.league, game.teamHome, home)}</div>`
      );
    }).join('');
  ...
  function renderStandings(root, data, state) {
    const blocks = Router.LEAGUES.map((league) => {
      const teams = Sorter.sortByUserPriority(data.teams.filter((team) => team.league === league).map((team) => ({ ...team, teams: [team.id] })), state);
      const lines = teams.map((team, index) => `<tr><td>${index + 1}</td><td>${escapeHtml(team.name)}</td><td>${team.trending ? '🔥' : '-'}</td></tr>`).join('');
      return `<article class="card" id="${league}"><div class="card-body"><h3 class="title small">${league}</h3><div class="tablewrap"><table><thead><tr><th>#</th><th>Time</th><th>Trend</th></tr></thead><tbody>${lines}</tbody></table></div></div></article>`;
    }).join('');
  ...
  function renderTeam(root, data, state, query) {
    ...
    root.innerHTML = `
      <section class="section"><div class="section-head"><div><h2>${team.name}</h2><p>${league}</p></div></div>
        <div class="pills">
          <button class="pill ${favorite ? 'active' : ''}" id="favoriteTeamBtn">⭐ Favoritar</button>
          <button class="pill ${followed ? 'active' : ''}" id="followTeamBtn">➕ Seguir</button>
        </div>
      </section>
      ...
      <section class="section"><div class="section-head"><div><h2>Estatísticas básicas</h2></div></div><div class="card"><div class="card-body"><p class="desc">Ataque: 8.4 • Defesa: 7.9 • Momento: ${team.trending ? 'Em alta' : 'Estável'} (mock)</p></div></div></section>
    `;
  }
  ...
  function renderSettings(root, data, state) {
    ...
    const followed = Router.LEAGUES.map((league) => {
      const selected = state.followedTeamsByLeague[league] || [];
      const pills = data.teams.filter((team) => team.league === league).map((team) => `<button class="pill ${selected.includes(team.id) ? 'active' : ''}" data-follow-team="${league}:${team.id}">${team.name}</button>`).join('');
      return `<div class="section"><div class="section-head"><div><h2>Seguidos ${league}</h2></div></div><div class="pills">${pills}</div></div>`;
    }).join('');

    root.innerHTML = `<section class="section"><div class="section-head"><div><h2>Configurações</h2></div></div><div class="card"><div class="card-body">${leagueOptions}${favoriteTeams}<button class="pill" id="resetPreferences">Resetar preferências</button></div></div></section>${followed}`;
  }
